...
s = p.search('"It is true for all that that that that that'
             ' that that refers to is not the same that that that that refers to..."').group()
print(s)
## https://www.cristinacabal.com/?p=482
p = re.compile(r'\b(\w+(\s*|[.]))(\1)+\b')
f = p.search('"I think that that that that that teacher gave us is correct...."').group().split()
print(f)
